...
class RatingDB
{
    ...
    public static function addRating($productRating)
    {
        ...
        $query = "INSERT INTO products_ratings(Product_Id,Rating,Customer_Id)
    ...
    {
        $db = database::getDB();
        $query = "SELECT Product_Id,AVG(Rating) AS rate FROM products_ratings where Product_Id = '$id'";
        $result = $db->query($query);
        $rate = $result->fetch();
        return $rate;
    }
    public static function getcount($Id)
    {
        $db = database::getDB();
        $prod_Id = $Id;
        $query = "SELECT COUNT(*) FROM products_ratings WHERE Product_Id = '$prod_Id' AND Rating IS NOT NULL ";
        $res = $db->query($query);
        $records = $res->fetchColumn() ;
        return $records;
    }
